...
from .helpers.structured_shareholder_information import StructuredInformation

# Initialize OpenAI client with your API key


@functions_framework.http
def get_shareholder_details(request):
    request_json = request.get_json(silent=True)
    request_args = request.args
    
    if request_json and 'shareholder_id' in request_json:
        df = request_json['shareholder_id']
    elif request_args and 'shareholder_id' in request_args:
        df = request_args['shareholder_id']
    else:
        return 'No data provided', 400
    
    structurer = StructuredInformation()
    result = structurer.get_shareholder_details_from_si(shareholder_id=df)
